...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    Integer countAllByUserAccount(Long userId);
    ...
    List<Ticket> fetchAllTicketsByUserAccount(@Param("id") Long id);

    //Write a JPQL query that returns all tickets between a range of dates
    @Query("SELECT t FROM Ticket t WHERE t.dateTime BETWEEN ?1 AND ?2")
    List<Ticket> fetchAllTicketsWithRangeOfDates(@Param("dateTime1") LocalDateTime dateTime1,
    ...
    Integer countTicketsByUser(@Param("userId") Long userId);

    //Write a native query to count the number of tickets a user bought in a specific range of dates
    @Query(value = "SELECT COUNT(* ticket WHERE user_account_id = ?1 AND " +
            "date_time BETWEEN ?2 AND ?3)",nativeQuery = true)
    Integer countTicketsByUserInDateRange(@Param("userId") Long userId,
                                          @Param("dateTime1") LocalDateTime dateTime1,
                                          @Param("dateTime2") LocalDateTime dateTime2);

    //Write a native query to distinct all tickets by movie name
    @Query(value = "SELECT DISTINCT(m.name) FROM ticket JOIN movie_cinema mc ON mc.id = t.movie_cinema_id " +
    ...
    @Query(value = "SELECT * FROM ticket t JOIN user_account ua ON t.user_account_id " +
            ...
            "WHERE ua.username ILIKE concat('%', ?1, '%') OR ad.name ILIKE concat('%', ?1, '%') " +
